...
describe('presentation journey history', () => {
  ...
  it('labels the presentation-only week as care context', () => {
  ...
  it('preserves valid zero readings and recorded zero activity', () => {
    ...
    expect(completed.lifestyle.hydration_target_ml_per_day).toBe(1500);
    expect(completed.checkIn.waterDailyTotalMl).toBe(0);
    expect(completed.sourceFlags.motherBabyContext).toBe(false);
  });

  it('adds missing hydration without inventing symptoms or a recorded check-in', () => {
    const completed = completeTodayPresentation({
    ...
    expect(completed.checkIn.waterDailyTotalMl).toBe(952);
    expect(completed.airExposure.pm25).toBe(42.7);
    expect(completed.airExposure.aqi).toBeUndefined();
  ...
  it('keeps an explicit symptom-free check-in ahead of scenario history', () => {
    ...
    expect(summary.days[6].symptomLabel).toBe('No symptoms reported');
